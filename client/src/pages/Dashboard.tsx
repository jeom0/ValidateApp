...
import { Users, Ticket, CheckCircle2, Calendar, AlertCircle, Clock } from 'lucide-react';
        ...
        {loading ? (
              ...
              {activities.map((act, i) => (
                <div key={act.id} style={{
                  ...
                  <div style={{
                    ...
                    background: act.resultado === 'valida' ? '#f0fdf4' : '#fef2f2',
                    color: act.resultado === 'valida' ? '#16a34a' : '#dc2626',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0
                  }}>
                    {act.resultado === 'valida' ? <CheckCircle2 size={20} /> : <AlertCircle size={20} />}
                  </div>
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: 0, fontWeight: 800, fontSize: '0.9375rem', color: '#111827' }}>
                      {act.clientName || 'Usuario Desconocido'} 
                    ...
                    <p style={{ margin: 0, fontSize: '0.8125rem', color: '#6b7280', fontWeight: 600 }}>
                      {act.eventName || 'Evento'} · {act.resultado === 'valida' ? 'Escaneo Exitoso' : 'Escaneo Fallido'}
                    </p>
                  </div>
                  <div style={{ textAlign: 'right', display: 'flex', alignItems: 'center', gap: '0.375rem', color: '#9ca3af', fontWeight: 600, fontSize: '0.75rem' }}>
                    <Clock size={12} />
                    {new Date(act.fecha_hora).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="card" style={{
            ...
            <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Ticket size={28} style={{ color: '#d1d5db' }} />
            </div>
